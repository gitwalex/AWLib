...
public enum AWLibDateSelector {
     ...
     *         ResID des Klartextes fuer die jeweilige Enum
     */
    AWLibDateSelector(int textResID) {
    ...
    public static String[] getDateTexte() {
        if (dateTexte == null) {
            dateTexte = new String[AWLibDateSelector.values().length];
            for (AWLibDateSelector ds : AWLibDateSelector.values()) {
                dateTexte[ds.ordinal()] = AWLIbApplication.getContext().getString(ds.textResID);
            }
        }
        return dateTexte;
    }
