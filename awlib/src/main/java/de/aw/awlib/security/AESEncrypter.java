...
public final class AESEncrypter {
    ...
    private static final byte[] SALT =
            {(byte) 0xA9, (byte) 0x9B, (byte) 0xC8, (byte) 0x32, (byte) 0x56, (byte) 0x35,
                    (byte) 0xE3, (byte) 0x03};
    private static AESEncrypter mDefaultEncrypter;
     ...
     */
    private AESEncrypter(byte[] key) throws Exception {
     ...
     * AESEncrypter#getDefault()} der entsprechende Encrypter besorgt werden. Sonst false.
     */
    public static boolean checkPassword(@NonNull String passPhrase, @NonNull byte[] mKey) {
        ...
        try {
            ...
            if (Arrays.equals(mKey, generatedKey)) {
                mDefaultEncrypter = new AESEncrypter(mKey);
                checked = true;
            }
        } catch (Exception e) {
     ...
     * AESEncrypter#generateKey(String)} erstellt werden. Sinnvoll z.B. bei Passwortwechsel
     *
     * @param key
     *         key
     *
     ...
     */
    public static AESEncrypter createInstance(byte[] key) throws Exception {
        mDefaultEncrypter = new AESEncrypter(key);
     ...
     * AESEncrypter#checkPassword(String, byte[])} mit true beendet wurde
     */
    public static AESEncrypter getDefault() {
        return mDefaultEncrypter;
    }
